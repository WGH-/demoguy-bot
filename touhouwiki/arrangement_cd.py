...
import logging
module_logger = logging.getLogger("touhohwiki.arrangement_cd")
...
class ArrangementCDParser(object):
    song_title_re = map(re.compile, [
            ur"(?u)(?P<track>\w+)\. *?(?P<title>.+?) \((?P<length>[\d:?-]*)\)$",
            ur"(?u)(?P<track>\w+)\. *?(?P<title>.+?)$",
    ])
    ...
    def __init__(self):
        self.wikidb = templates.WikiDB()
        self.logger = logging.getLogger("touhohwiki.arrangement_cd.ArrangementCDParser")
